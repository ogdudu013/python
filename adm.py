...
def baixar_yt(busca):
    ...
    with yt_dlp.YoutubeDL(ydl_opts) as ydl:
        ...
        nome_audio = f"audio_{timestamp}.{ext}"
        nome_txt = f"data_{timestamp}.txt"
        
        if os.path.exists(f"temp.{ext}"):
            os.rename(f"temp.{ext}", nome_audio)
...
def subir_ftp(arquivo, pasta):
    try:
        ftp = FTP(FTP_HOST); ftp.login(user=FTP_USER, passwd=FTP_PASS)
        ftp.set_pasv(True); ftp.cwd(pasta)
        with open(arquivo, 'rb') as f: ftp.storbinary(f'STOR {arquivo}', f)
        ftp.quit()
        return True
    except Exception as e:
        print(f"❌ Erro FTP: {e}"); return False

if __name__ == "__main__":
    busca = input("Nome ou Link do YouTube: ")
    try:
        audio, txt = baixar_yt(busca)
        subir_ftp(audio, 'htdocs/uploads/songs')
        subir_ftp(txt, 'htdocs/uploads/queue')
        print("✅ Enviado! Processe no painel Admin.")
